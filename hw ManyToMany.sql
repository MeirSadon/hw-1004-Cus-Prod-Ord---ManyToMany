...
select  c.name as cusName, avg(price) as avger from                                                                                     ORDERS o join Customer c on c.id=o.CUSTOMER_ID join Products p on o.PRODUCT_ID=p.id group by c.name having avger>(
select avg(price) from ORDERS o join products p on o.PRODUCT_ID=p.id join customer c on o.CUSTOMER_ID=c.id
)
                                                                                                                                          // מצא את סכום כל ההזמנות                                                                                                             select sum(price) from                                                                                                                   ORDERS o join Customer c on c.id=o.CUSTOMER_ID join Products p on o.PRODUCT_ID=p.id
